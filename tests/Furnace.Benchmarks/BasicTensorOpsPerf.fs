...
module PythonResults =
    let pythonResults = 
        let pyFile = Path.Combine(__SOURCE_DIRECTORY__, "..", "Furnace.Benchmarks.Python", "results.csv")
        if File.Exists(pyFile) then  
            ...
            printfn "*** No python results found at '%s', have you run Furnace.Benchmarks.Python?" pyFile
            dict [ ]
