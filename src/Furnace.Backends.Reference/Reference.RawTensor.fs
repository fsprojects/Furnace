...
namespace rec Furnace.Backends.TestDuplicate
#else
namespace rec Furnace.Backends.Reference
#endif
...
module internal RawTensorCPU = 
    ...
    let inline Random ofDouble (shape:Shape) : (^T[] * Shape) =
        let values = Array.init (shapeLength shape) (fun _ -> ofDouble (Furnace.Util.Random.Uniform()))
        (values, shape)

    let inline RandomNormal ofDouble (shape:Shape) : (^T[] * Shape) =
        let values = Array.init (shapeLength shape) (fun _ -> ofDouble (Furnace.Util.Random.Normal()))
        (values, shape)

    let inline RandomInt ofInt (shape:Shape) (low:int) (high:int) : (^T[] * Shape) =
        let values = Array.init (shapeLength shape) (fun _ -> ofInt (Furnace.Util.Random.Integer(low, high)))
        (values, shape)
