...
namespace rec Furnace.Backends.Torch
...
module internal Utils = 
    ...
    let fromTorchDevice (x: torch.Device) = Furnace.Device(fromTorchDeviceType x.``type``, x.index)
...
type TorchRawTensor(tt: torch.Tensor, shape: Shape, dtype: Dtype, device: Device) =
    ...
    override _.DeviceType : Furnace.DeviceType = enum (int tt.device_type)
    override t.Device = Furnace.Device(t.DeviceType, tt.device_index)
    ...
    override t.GatherT(dim:int, indices) =
        ...
        // NOTE: Furnace currently expects indices as an Int32 tensor, Torch wants Int64
    ...
    override t1.DivTT(t2) = 
        ...
        // also see https://github.com/Furnace/Furnace/issues/239
    ...
    override t1.MatMulTT(t2) = 
        ...
        let result =
            ...
            | Furnace.DeviceType.CUDA, (Dtype.Integral as dtype) ->
    ...
    override t1.MaxPool1D(kernelSize, stride, padding) = 
        ...
        // NOTE: Furnace currently expects indices as an Int32 tensor
...
type TorchBackendTensorStatics() =
    ...
    let isSupported (deviceType: Furnace.DeviceType) = 
    ...
    override _.GetDevices(deviceType) = 
        [ 
          ...
          | None | Some Furnace.DeviceType.CPU ->
              yield Device.CPU
          | _ -> ()

          match deviceType with
          | None | Some Furnace.DeviceType.CUDA ->
              if torch_cuda.is_available() then 
                  let ncuda = torch_cuda.device_count()
                  for i in 0 .. ncuda - 1 do
                      yield (Furnace.Device(Furnace.DeviceType.CUDA, i))
          | _ -> ()
    ...
    override _.IsDeviceTypeAvailable (deviceType) =
        match deviceType with 
        | Furnace.DeviceType.CPU -> true
        | Furnace.DeviceType.CUDA -> torch_cuda.is_available()
